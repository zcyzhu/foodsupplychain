...
describe('ProducerComponent', () => {
  ...
  it('should update the table when a Producer is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceProducer, 'addParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Producer is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceProducer, 'updateParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Producer is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceProducer, 'deleteParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
